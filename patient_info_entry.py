...
def build_rag_chain(vectordb, mode="pre"):
    ...
    if mode == "pre":
        template = """
你是一位资深的临床营养师和健康管理师，专攻基于“肠道菌群紊乱/肠屏障受损-慢性炎症-表观遗传”机制运用营养治疗进行糖尿病营养管理，擅长通过科学合理的应用武汉英纽林的系列富含多种益生元的功能营养食品及中药膳营养产品，调节糖尿病患者肠道菌群，促进丁酸发酵，修复肠屏障阻断慢性炎症，促进患者自身的GLP-1分泌，并通过调节肠道菌群发酵产物调节表观遗传修饰，实现对高糖代谢记忆的缓解，从而逆转糖尿病患者的胰岛素抵抗及胰岛素分泌障碍问题。你的应用，为当前患者制定个体化的干预方案，并分五部分清晰作答：
...
"""
        ...
        template = """
你是一位富有亲和力的临床营养师和健康管理师，擅长用积极、鼓励的方式解读英纽林系列营养产品对糖尿病营养治疗的效果，特别是说明通过补充各产品后，通过有效的调节糖尿病患者肠道菌群，促进丁酸发酵，修复肠屏障阻断慢性炎症，促进患者自身的GLP-1分泌，并通过调节肠道菌群发酵产物调节表观遗传修饰，实现对高糖代谢记忆的缓解，从而逆转糖尿病患者的胰岛素抵抗及胰岛素分泌障碍问题的效果。请根据以下资料，为这位患者进行干预前后对比分析，并给出下阶段建议。
    ...
    if not display_patients:
        ...
        if show_symptom:
            ...
            if not pre_symptom:
                ...
                fig.update_layout(title=f"{selected_patient_name} - 体感评分对比",
                                  xaxis_title="体感项目", yaxis_title="评分 (0最差, 10最好)",
                                  barmode='group', yaxis=dict(range=[0, 10]))
                st.plotly_chart(fig, use_container_width=True)
        ...
        if show_biochem:
            ...
            for field_name, pre_key, post_key, unit in bio_fields:
                ...
                fig.update_layout(title=f"{selected_patient_name} - {field_name}",
